			...
			<div class="page-content">
				...
				<div class="row">
					<div class="col-xs-12">
						...
						<div class="row">
							<div class="col-xs-12">
								<button id="btn_new" name="btn_new" class="btn btn-success btn-xs" onclick="web_reload()">
						...
						$repack_ptid_id = trim($this->input->post('repack_ptid_id'));
						$repack = trim($this->input->post('repack'));
						$get_data = $this->M_library_database->DB_GET_DATA_ALL_PRODUCT_INBOUND_DETAIL_BY_PRODUCT_Q($repack_ptid_id);
						...
						foreach ($get_data as $get_row) {
							$PTIDDL_ID = $get_row->PTIDDL_ID;
							$PTIDDL_DATE = $get_row->PTIDDL_DATE;
							$PTID_ID = $get_row->PTID_ID;
							$PT_ID = $get_row->PT_ID;
							$PT_NAME = $get_row->PT_NAME;
							$PT_UNIT = $get_row->PT_UNIT;
							$PT_TYPE = $get_row->PT_TYPE;
							$PT_IS_MANUFDATE = $get_row->PT_IS_MANUFDATE;
							$RK_ID = $get_row->RK_ID;
							$PTIDDL_LABEL = $get_row->PTIDDL_LABEL;
							$PTIDDL_NO = $get_row->PTIDDL_NO;
							$PTIDDL_MANUFDATE = $get_row->PTIDDL_MANUFDATE;
							$PTIDDL_EXPIRED = $get_row->PTIDDL_EXPIRED;
							$SR_ID = $get_row->SR_ID;
							$SR_NAME = $get_row->SR_NAME;
						}

						?>
						<form action="<?php echo site_url('C_i_input_data_quarantine/data_insert_q'); ?>" method="post" enctype="multipart/form-data">
							<!------------------------------------------------------------------------------------------------->					
							<div class="row">
								<div class="col-xs-12">
									<div class="col-xs-12 col-sm-3">
										<div class="form-group">
											...
											<input type="type" id="si_id" name="si_id" class="form-control" value="<?php echo $this->M_library_module->GENERATOR_DOCID_INBOUND(); ?>" required="required" readonly="readonly"/>
										</div>
									</div>
								</div>
							</div>
									...
									<div class="col-xs-12 col-sm-3">
										<div class="form-group">
											...
											<select class="chosen-select form-control" id="si_supplier_id" name="si_supplier_id" data-placeholder="Please Choose" required="required"/>
												...
												<option selected value="<?php echo $SR_ID; ?>"><?php echo $SR_NAME; ?></option>
											</select>
										</div>
									</div>
									<div class="col-xs-12 col-sm-3">
										<div class="form-group">
											...
											<input type="text" id="si_po_no" name="si_po_no" class="form-control" maxlength="150" autocomplete="off" onkeyup="auto_caps(this)" required="required"/>
										</div>
									</div>
									<div class="col-xs-12 col-sm-3">
										<div class="form-group">
											<label for="si_packing_list_no" class="control-label">Packing List No</label>
											<input type="text" id="si_packing_list_no" name="si_packing_list_no" class="form-control" maxlength="150" autocomplete="off" onkeyup="auto_caps(this)" required="required"/>
										</div>
									</div>
									<div class="col-xs-12 col-sm-3">
										<div class="form-group">
											<label for="si_pib_no" class="control-label">PIB No</label>
											<input type="text" id="si_pib_no" name="si_pib_no" class="form-control" maxlength="150" autocomplete="off" onkeyup="auto_caps(this)" required="required"/>
										</div>
									</div>
									<div class="col-xs-12 col-sm-3">
										<div class="form-group">
											<label for="si_invoice_no" class="control-label">Invoice No</label>
											<input type="text" id="si_invoice_no" name="si_invoice_no" class="form-control" maxlength="150" autocomplete="off" onkeyup="auto_caps(this)" required="required"/>
										</div>
									</div>
									<div class="col-xs-12 col-sm-3">
										<div class="form-group">
											<label for="si_forwarder_id" class="control-label">Forwarder</label>
											<select class="chosen-select form-control" id="si_forwarder_id" name="si_forwarder_id" data-placeholder="Please Choose" required="required"/>
												<option value="">Please Choose</option>
												<?php
...
<td><input name="si_product_ismanufdate" type="text" value="<?php echo $PT_IS_MANUFDATE ?>" class="form-control si_product_ismanufdate" required="required" readonly="readonly"/></td>
<td><input type="text" id="si_product_qty" name="si_product_qty" class="form-control" maxlength="20" autocomplete="off" onkeydown="numbers_only(event)" required="required" value="<?php echo $repack; ?>" /></td>
																<td>
																	<div class="input-group">
																		<input class="form-control date-picker" id="si_product_expired" name="si_product_expired" type="text" data-date-format="yyyy-mm-dd" autocomplete="off" />
																		<span class="input-group-addon">
																			<i class="fa fa-calendar bigger-110"></i>
																		</span>
																	</div>
									...
									<p>
										<!-- <a id="btn_new_row" name="btn_new_row" class="btn btn-info btn-xs">
											<i class="ace-icon fa fa-plus"></i>
											Add
										</a>
										<a id="btn_delete_row" name="btn_delete_row" class="btn btn-danger btn-xs">
											<i class="ace-icon fa fa-trash"></i>
											Del
										</a> -->
										<button id="btn_next" name="btn_next" class="btn btn-success btn-xs pull-right">
...
<script type="text/javascript">
	...
	//-----------------------------------------------------------------------------------------------//
	//dynamic table
	function select_value_product(id){
		el='#'+id;
		var myArray = JSON.parse('<?php echo json_encode($get_data_product); ?>');
		var collect = '<option value="">Please Choose</option>';
		for(var i = 0; i<myArray.length;i++){
			var PT_ID = myArray[i]["PT_ID"];
			var PT_NAME = myArray[i]["PT_NAME"];
			var PT_UNIT = myArray[i]["PT_UNIT"];
			var PT_UNIT_MAX = myArray[i]["PT_UNIT_MAX"];
			var PT_TYPE = myArray[i]["PT_TYPE"];
			var PT_IS_MANUFDATE = myArray[i]["PT_IS_MANUFDATE"];
			var SR_ID = myArray[i]["SR_ID"];
			var SR_NAME = myArray[i]["SR_NAME"];
			
			var BUILD_DATA = PT_ID+"[]"+PT_NAME+"[]"+PT_UNIT+"[]"+PT_UNIT_MAX+"[]"+PT_TYPE+"[]"+PT_IS_MANUFDATE+"[]"+SR_ID+"[]"+SR_NAME;
			
			collect += '<option value="'+BUILD_DATA+'">'+PT_ID+'</option>';
		}
		$(el+" select").html(collect);
	}
	
	$(document).ready(function(){
		select_value_product('addr0');
			
			var i=1;
		$("#btn_new_row").click(function(){
			b=i-1;
			$('#addr'+i).html($('#addr'+b).html()).find('td:first-child').html(i+1);
			$('#tb_item').append('<tr id="addr'+(i+1)+'"></tr>').find('.date-picker').datepicker();
			select_value_product('addr'+i);
			i++; 
		});
	...
	function product_info(pointer){
		...
		$(pointer).closest("tr").find(".si_product_type").val(array[4]);
		$(pointer).closest("tr").find(".si_product_ismanufdate").val(array[5]);
		//$(pointer).closest("tr").find(".si_supplier_id").val(array[6]);//???
		//$(pointer).closest("tr").find(".si_supplier_name").val(array[7]);//???
	}

	// $('#si_supplier_id').change(function(){
	// 		var id_key = document.getElementById("si_supplier_id");
	// 		id_key = id_key.options[id_key.selectedIndex].value;
	// 		// alert(id_key);
	// 		//AJAX Request
	// 		$.ajax({
	// 			url: '<?= site_url('C_i_input_data/get_produk_by_id') ?>',
	// 			type: 'POST', 
	// 			data: {id_key: id_key},
	...
	// 				// $('#si_product_id[]').find('option').not(':first').remove();

	// 				//Add Options
	// 				$.each(response,function(index,data){
	// 					$('#si_product_id[]').append('<option value="'+data['PT_ID']+'">'+data['PT_ID']+'</option>');
	// 				});
	// 			}
	// 		});
	// 	});
