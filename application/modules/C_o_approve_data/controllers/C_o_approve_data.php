...
class C_o_approve_data extends CI_Controller
{
	...
	public function data_approve()
	{
		...
		$array_si_detail_id_inbound = $_POST['si_detail_id_inbound'];
		...
		$array_si_product_qty_stock = $_POST['si_product_qty_stock'];
		...
		for ($x = 0; $x < $array_si_detail_id_length; $x++) {
			...
			$si_detail_id_inbound = $array_si_detail_id_inbound[$x];
			$si_detail_date = date('Y-m-d H:i:s');
			$si_product_detail_id = $array_si_product_detail_id[$x];
			$si_product_no = $array_si_product_no[$x];
			$si_product_qty_total = $array_si_product_qty_total[$x];
			$si_product_qty_stock = $array_si_product_qty_stock[$x];
			$si_product_qty_good = $array_si_product_qty_good[$x];
			$si_product_qty_bad = $array_si_product_qty_bad[$x];

			$total_stock = $si_product_qty_stock - $si_product_qty_total;
			//-----------------------------------------------------------------------------------------------//
			$data_update = array(
				...
				'PTODDL_STOCK_INBOUND' => $total_stock,
			...
			$update_inbound = array('PTIDDL_QTY_GOOD' => $total_stock );
			$is_ok = $this->M_library_database->DB_UPDATE_DATA_PRODUCT_INBOUND_DETAIL($si_detail_id_inbound, $update_inbound);
			// if ($is_ok) {
			// 	$
			// }
			//-----------------------------------------------------------------------------------------------//
			if ($total_stock == 0) {
				$is_ok = $this->M_library_database->DB_DELETE_PRODUCT_INBOUND_DETAIL_EVO($si_detail_id_inbound);
			}
		}
