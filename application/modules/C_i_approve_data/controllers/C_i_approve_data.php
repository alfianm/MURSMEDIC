    ...
    public function data_approve()
    {
        ...
        for ($y = 0; $y < $array_si_detail_id_length; $y++) {
            ...
            foreach ($get_data_search_product_for_rack_evo_filter as $data_row_search_product_for_rack_evo_filter) {
                ...
                $SE_ID_PRODUCT = $data_row_search_product_for_rack_evo_filter->SE_ID;
            }
        ...
        $get_data_all_rack_filter = $this->M_library_database->DB_GET_DATA_ALL_RACK($SE_ID_PRODUCT);
        // print_r($get_data_all_rack_filter);
        // die();
        ...
        if (empty($get_data_all_rack_filter)) {
            $ALL_RACK = 0;
        } else {
            $ALL_RACK = count($get_data_all_rack_filter);
        }

        $check_rack_inbound_detail_evo_filter = $this->M_library_database->DB_CHECK_RACK_INBOUND_DETAIL_DRIVE_IN_EVO($SE_ID_PRODUCT);

        if (empty($check_rack_inbound_detail_evo_filter)) {
            $RACK_INBOUND_DETAIL = 0;
        } else {
            $RACK_INBOUND_DETAIL = count($check_rack_inbound_detail_evo_filter);
        }

        // $check_rack_outbound_detail_evo_filter = $this->M_library_database->DB_CHECK_RACK_OUTBOUND_DETAIL_DRIVE_IN_EVO($SE_ID_PRODUCT);

        // if (empty($check_rack_outbound_detail_evo_filter)) {
        //     $RACK_OUTBOUND_DETAIL = 0;
        // } else {
        //     $RACK_OUTBOUND_DETAIL = count($check_rack_outbound_detail_evo_filter);
        // }

        $RACK_AVAILABLE = $ALL_RACK - $RACK_INBOUND_DETAIL;

        //-----------------------------------------------------------------------------------------------//
        if ($RACK_USE_FILTER > $RACK_AVAILABLE) {
            echo '
				<script>
					alert("APPROVE FAILED, RACK USE ' . $RACK_USE_FILTER, ' AND RACK AVAILABLE ' . $RACK_AVAILABLE . ' PLEASE ADD MORE RACK");
					window.location.href = "' . site_url('i_approve_data') . '";
				</script>
			';
            exit();
        ...
        for ($x = 0; $x < $array_si_detail_id_length; $x++) { //LOOP 1
            ...
            foreach ($get_data_search_product_for_rack_evo as $data_row_search_product_for_rack_evo) {
                ...
                $SE_ID = $data_row_search_product_for_rack_evo->SE_ID;
            ...
            if (!empty($si_product_qty_bad) || !$si_product_qty_bad == 0) {
                ...
                $data_quarantine = [
                    ...
                    'PT_ID' => $INFO_PT_ID,
                    'PTQE_NAME' => $si_product_detail_name,
                    'PTQE_DATE' => date('Y-m-d H:i:s'),
                    'PTQE_NO' => $si_product_no,
                    'PTQE_EXPIRED' => $INFO_PTIDDL_EXPIRED,
                    'PTQE_QTY' => $si_product_qty_bad,
                    'PTQE_STATUS' => "WAITING"
                ];
            ...
            //-----------------------------------------------------------------------------------------------//
            $get_data_all_rack = $this->M_library_database->DB_GET_DATA_ALL_RACK($SE_ID_USE);
