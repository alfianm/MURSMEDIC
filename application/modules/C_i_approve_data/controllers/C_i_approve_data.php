...
class C_i_approve_data extends CI_Controller
{
	...
	public function data_approve()
	{
		...
		$array_si_detail_no_length = count($array_si_product_no);
		//-----------------------------------------------------------------------------------------------//
		...
		for ($y = 0; $y < $array_si_detail_id_length; $y++) {
			...
			$RACK_USE_COUNT_FILTER_1 = $RACK_USE_COUNT_FILTER;
		}



		$RACK_USE_POST = $RACK_USE_COUNT_FILTER_1;
		if ($RACK_USE_POST == 1) {
			$get_data_all_rack_post = $this->M_library_database->DB_GET_DATA_RACK_WHERE_TYPE_LEVEL_1();
			if (empty($get_data_all_rack_post) || $get_data_all_rack_post == "") {
				echo '
				<script>
			...
			foreach ($get_data_all_rack_post as $data_row_all_rack_filter) {
		...
		} elseif ($RACK_USE_POST == 2) {
			$get_data_all_rack_post = $this->M_library_database->DB_GET_DATA_RACK_WHERE_TYPE_LEVEL_2();
			if (empty($get_data_all_rack_post) || $get_data_all_rack_post == "") {
				echo '
				<script>
					alert("APPROVE FAILED, RACK NOT FOUND WHEN FILTER");
			...
			//-----------------------------------------------------------------------------------------------//
				$PREFIX_RACK_ID_FILTER = substr($RK_ID_FILTER, 0, 3);
			//-----------------------------------------------------------------------------------------------//
				$check_rack_inbound_detail_evo_filter = $this->M_library_database->DB_CHECK_RACK_INBOUND_DETAIL_DRIVE_IN_EVO($RK_ID_FILTER);
				if (empty($check_rack_inbound_detail_evo_filter) || $check_rack_inbound_detail_evo_filter == "") {
					$RACK_AVAILABLE_FILTER = $RACK_AVAILABLE_FILTER + 1;
				} else {
					$check_rack_outbound_detail_evo_filter = $this->M_library_database->DB_CHECK_RACK_OUTBOUND_DETAIL_DRIVE_IN_EVO($PREFIX_RACK_ID_FILTER);
		...
		} elseif ($RACK_USE_POST == 3 || $RACK_USE_POST == 4 || $RACK_USE_POST == 5) {
			$get_data_all_rack_post = $this->M_library_database->DB_GET_DATA_RACK_WHERE_TYPE_LEVEL_3();
			if (empty($get_data_all_rack_post) || $get_data_all_rack_post == "") {
				echo '
				<script>
					alert("APPROVE FAILED, RACK NOT FOUND WHEN FILTER");
			...
			//-----------------------------------------------------------------------------------------------//
				...
				if (empty($check_rack_inbound_detail_evo_filter) || $check_rack_inbound_detail_evo_filter == "") {
					...
					$check_rack_outbound_detail_evo_filter = $this->M_library_database->DB_CHECK_RACK_OUTBOUND_DETAIL_DRIVE_IN_EVO($RK_ID_FILTER);
					if (empty($check_rack_outbound_detail_evo_filter) || $check_rack_outbound_detail_evo_filter == "") {
						$get_data_detail_filter = $this->M_library_database->DB_GET_DATA_SEARCH_DETAIL_RACK_STOCK($RK_ID_FILTER);
						foreach ($get_data_detail_filter as $data_row_detail_filter) {
							$RK_ID_FILTER_STOCK = $data_row_detail_filter->RK_ID;
			...
			$get_data_all_rack_post = $this->M_library_database->DB_GET_DATA_RACK_WHERE_TYPE_LEVEL_4();
			if (empty($get_data_all_rack_post) || $get_data_all_rack_post == "") {
				echo '
				<script>
					alert("APPROVE FAILED, RACK NOT FOUND WHEN FILTER");
			...
			foreach ($get_data_all_rack_post as $data_row_all_rack_filter) {
				...
				$RK_LEVEL_FILTER = $data_row_all_rack_filter->RK_LEVEL;
				$SE_ID_FILTER = $data_row_all_rack_filter->SE_ID;
				$RK_STATUS_FILTER = $data_row_all_rack_filter->RK_STATUS;
			//-----------------------------------------------------------------------------------------------//
				$PREFIX_RACK_ID_FILTER = substr($RK_ID_FILTER, 0, 3);
			//-----------------------------------------------------------------------------------------------//
				$check_rack_inbound_detail_evo_filter = $this->M_library_database->DB_CHECK_RACK_INBOUND_DETAIL_DRIVE_IN_EVO($PREFIX_RACK_ID_FILTER);
		...
		// var_dump($RACK_USE_FILTER . "<>" . $RACK_AVAILABLE_FILTER);
		// die();
		//FILTER ALL SUM PRODUCT & FILTER SUM USE RACK & COMPARE AVAILABLE RACK
		//-----------------------------------------------------------------------------------------------//
		...
		for ($x = 0; $x < $array_si_detail_id_length; $x++) {//LOOP 1
			...
			if (!empty($si_product_qty_bad) || !$si_product_qty_bad == 0) {

				for ($i = 0; $i < $array_si_detail_no_length; $i++) {

					$data_quarantine = [
						'PTQE_ID' => $this->M_library_module->GENERATOR_REFF(),
						'PTIDDL_ID' => $INFO_PTIDDL_ID,
						'PTID_ID' => $INFO_PTID_ID,
						'PT_ID' => $INFO_PT_ID,
						'PTQE_NAME' => $array_si_product_detail_name[$i],
						'PTQE_DATE' => date('Y-m-d H:i:s'),
						'PTQE_NO' => $array_si_product_no[$i],
						'PTQE_EXPIRED' => $INFO_PTIDDL_EXPIRED,
						'PTQE_QTY' => $array_si_product_qty_bad[$i],
						'PTQE_STATUS' => "WAITING"
					];
					$is_ok = $this->M_library_database->DB_INSERT_DATA_PRODUCT_QUARANTINE($data_quarantine);
				}
			}
			...
			if ($FINAL_RACK_USE == 1) {
				$get_data_all_rack_type_level_1 = $this->M_library_database->DB_GET_DATA_RACK_WHERE_TYPE_LEVEL_1();
				if (empty($get_data_all_rack_type_level_1) || $get_data_all_rack_type_level_1 == "") {
					echo '
					<script>
						alert("APPROVE FAILED, RACK NOT FOUND");
						window.location.href = "' . site_url('i_approve_data') . '";
				...
				foreach ($get_data_all_rack_type_level_1 as $data_row_all_rack) {//LOOP 2
				...
				//-----------------------------------------------------------------------------------------------//
					if (empty($check_rack_inbound_detail_evo) || $check_rack_inbound_detail_evo == "") {
						if ($si_product_no == "") {
							$barcode = substr($si_product_detail_id, 0, 6) . "NONE" . date('Ymd') . $this->M_library_module->CLEAN_STRING(substr($si_date_receive, 0, 6)) . rand(111, 999);
						} else {
							$barcode = substr($si_product_detail_id, 0, 6) . substr($si_product_no, 0, 4) . date('Ymd') . $this->M_library_module->CLEAN_STRING(substr($si_date_receive, 0, 6)) . rand(111, 999);
						}
			...
			} elseif ($FINAL_RACK_USE == 2) {
				$get_data_all_rack_type_level_2 = $this->M_library_database->DB_GET_DATA_RACK_WHERE_TYPE_LEVEL_2();
				if (empty($get_data_all_rack_type_level_2) || $get_data_all_rack_type_level_2 == "") {
					echo '
					<script>
						alert("APPROVE FAILED, RACK NOT FOUND");
						window.location.href = "' . site_url('i_approve_data') . '";
				...
				foreach ($get_data_all_rack_type_level_2 as $data_row_all_rack) {//LOOP 2
					$RK_ID = $data_row_all_rack->RK_ID;
					$RK_TYPE = $data_row_all_rack->RK_TYPE;
					$RK_SUBRACK = $data_row_all_rack->RK_SUBRACK;
					$RK_LEVEL = $data_row_all_rack->RK_LEVEL;
					$SE_ID = $data_row_all_rack->SE_ID;
					$RK_STATUS = $data_row_all_rack->RK_STATUS;
				//-----------------------------------------------------------------------------------------------//
					$PREFIX_RACK_ID = substr($RK_ID, 0, 3);
				//-----------------------------------------------------------------------------------------------//
					$check_rack_inbound_detail_evo = $this->M_library_database->DB_CHECK_RACK_INBOUND_DETAIL_EVO($RK_ID);
			...
			} elseif ($FINAL_RACK_USE == 3 || $FINAL_RACK_USE == 4 || $FINAL_RACK_USE == 5) {
				$get_data_all_rack_type_level_3 = $this->M_library_database->DB_GET_DATA_RACK_WHERE_TYPE_LEVEL_3();
				if (empty($get_data_all_rack_type_level_3) || $get_data_all_rack_type_level_3 == "") {
					echo '
					<script>
						alert("APPROVE FAILED, RACK NOT FOUND");
						window.location.href = "' . site_url('i_approve_data') . '";
				...
				foreach ($get_data_all_rack_type_level_3 as $data_row_all_rack) {//LOOP 2
					$RK_ID = $data_row_all_rack->RK_ID;
					$RK_TYPE = $data_row_all_rack->RK_TYPE;
					$RK_SUBRACK = $data_row_all_rack->RK_SUBRACK;
					$RK_LEVEL = $data_row_all_rack->RK_LEVEL;
				...
				$get_data_all_rack_type_level_4 = $this->M_library_database->DB_GET_DATA_RACK_WHERE_TYPE_LEVEL_4();
				if (empty($get_data_all_rack_type_level_4) || $get_data_all_rack_type_level_4 == "") {
					echo '
					<script>
						alert("APPROVE FAILED, RACK NOT FOUND");
						window.location.href = "' . site_url('i_approve_data') . '";
				...
				foreach ($get_data_all_rack_type_level_4 as $data_row_all_rack) {//LOOP 2
					$RK_ID = $data_row_all_rack->RK_ID;
					$RK_TYPE = $data_row_all_rack->RK_TYPE;
					$RK_SUBRACK = $data_row_all_rack->RK_SUBRACK;
					$RK_LEVEL = $data_row_all_rack->RK_LEVEL;
